...
public class SearchCandidate {
	...
	public void search(JComboBox<String> box, String date, int start, int end, int t1, int t2, int count, int side, int type, int early) {
		...
		try {
			//-1 表示沒有停
			//SELECT * FROM [direction] WHERE date = [date] AND [start] != -1 AND [end] != -1 AND [start] >= [t1] AND [end] <= t2",
			ret = MysqlExe.execQuery(String.format(
					"SELECT * FROM %s WHERE date = %s AND %s != -1 AND %s != -1 AND %s >= %d",
					direction, date.replace("/", ""), Station.ENG_NAME[start], Station.ENG_NAME[end], Station.ENG_NAME[start], t1
					));
			while (ret.res.next()) {
